<template>
  <q-card class="mycard zoom radius16">
    <router-link v-ripple class="cursor-pointer nounderline relative-position" :to="{
      ...
      <div id="cardimagebg" style="border-top-left-radius: 16px; border-top-right-radius: 16px">
        <q-img ratio="1" fit="contain" loading="lazy" :src="data.image"
      ...
      <q-card-section vertical>
        ...
        <q-chip color="primary" outline :icon="tipoToIcon(data.tipo)" v-if="data.tipo" :label="data.tipo"></q-chip>
        ...
        <q-chip class="mychip" v-for="item in data.tags" :color="item.color" :icon="item.icon" outline
          :key="item.label"><span class="mychipcontent">{{ item.label }}</span></q-chip>
      </q-card-section>
    </router-link>
  </q-card>
</template>
